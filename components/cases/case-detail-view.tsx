    ...
    <section aria-labelledby="case-title" className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            <p className="text-sm font-semibold tracking-[0.12em] text-primary uppercase">
              {t(locale, "cases.coordination")}
            </p>
            <h1 id="case-title" className="mt-2 text-balance text-3xl font-semibold tracking-tight sm:text-4xl">
              {caseRecord.title}
            </h1>
            <p className="mt-2 text-pretty text-muted-foreground">
              {caseRecord.outletName} · {caseRecord.provider.replace("_", " ")} · Owner: {caseRecord.owner}
            </p>
          </div>
      ...
      <Card className="border border-border shadow-none">
        <CardContent className="flex flex-col gap-3 pt-4 text-sm sm:flex-row sm:items-center sm:justify-between">
          <p className="font-medium">{t(locale, "cases.providerScoped")}</p>
          <div className="flex flex-wrap gap-4">
            <Link className="inline-flex items-center gap-1 text-primary hover:underline" href={alertHref}>
              <Link2Icon aria-hidden="true" className="size-4" />
              Source Alert
            </Link>
            <Link className="inline-flex items-center gap-1 text-primary hover:underline" href={outletHref}>
              <Link2Icon aria-hidden="true" className="size-4" />
              Outlet Detail
            </Link>
          </div>
