...
function AlertCard({ alert, detailBasePath }: { alert: OperationsAlert; detailBasePath: "/alerts" | "/preview/operations/alerts" }) {
      ...
      <CardHeader>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2"><Icon aria-hidden="true" className="size-5 shrink-0 text-primary" /><CardTitle className="text-balance">{presentation.label}</CardTitle></div>
            <CardDescription className="mt-2 text-pretty">{alert.outletName} · {alert.area} · {formatDateTime(alert.occurredAt)}</CardDescription>
          </div>
          <div className="flex flex-wrap gap-2"><Badge variant="outline" className={severityClass[alert.severity]}>{alert.severity}</Badge><FreshnessBadge freshness={alert.freshness} /></div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-pretty text-sm leading-6">{alert.summary}</p>
...
export async function AlertsView({ filters, action }: { filters: AlertFilters; action: "/alerts" | "/preview/operations/alerts" }) {
  ...
  const alerts = await getAlerts(filters.view === "empty" ? { ...filters, severity: "LOW", dataQuality: "critical" } : filters);

  return (
    <section aria-labelledby="alerts-title" className="mx-auto max-w-6xl space-y-6">
      <header className="border-b border-border pb-5">
        <p className="text-sm font-semibold tracking-[0.12em] text-primary uppercase">Review Queue</p>
        <h1 id="alerts-title" className="mt-2 text-balance text-3xl font-semibold tracking-tight sm:text-4xl">Alerts & Evidence</h1>
        <p className="mt-2 max-w-3xl text-pretty text-muted-foreground">Review liquidity pressure, unusual activity, and data consistency evidence. Signals remain separate unless an explicit combined review exists.</p>
      </header>
      <AlertFiltersBar action={action} filters={filters} />
      {alerts.length === 0 ? <EmptyState title="No Alerts Match These Filters" description="Change one or more filters to review alerts in your authorized provider scope." /> : <div className="space-y-4">{alerts.map((alert) => <AlertCard key={alert.id} alert={alert} detailBasePath={action} />)}</div>}
    </section>
  );
}
