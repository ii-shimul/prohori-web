...
export async function FeedHealthView({ filters }: { filters: FeedHealthFilters }) {
  ...
  const incidents = filters.view === "empty" ? [] : await getFeedHealthIncidents();
  ...
  return (
    <section aria-labelledby="feed-health-title" className="mx-auto max-w-6xl space-y-6">
      ...
      {incidents.length === 0 ? <EmptyState title="No Feed Incidents Match" description="No synthetic feed-health incidents are available for this view." /> : <div className="grid gap-4 lg:grid-cols-2">{incidents.map((incident) => <Card key={incident.id} className="border border-border shadow-none"><CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><DatabaseZapIcon aria-hidden="true" className="size-5 shrink-0 text-primary" /><CardTitle className="text-balance">{issueLabels[incident.issue]}</CardTitle></div><CardDescription className="mt-2 break-words">{incident.source} · {incident.affectedScope}</CardDescription></div><div className="flex shrink-0 flex-wrap gap-2"><FreshnessBadge freshness={incident.freshness} /><Badge variant="outline" className="border-border bg-muted text-foreground">Lag: {incident.lag}</Badge></div></div></CardHeader><CardContent className="space-y-4"><dl className="grid gap-3 rounded-lg bg-muted p-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">Observed</dt><dd className="mt-1 font-medium tabular-nums">{formatDateTime(incident.observedAt)}</dd></div><div><dt className="text-xs text-muted-foreground">Confidence Effect</dt><dd className="mt-1 font-medium">{incident.confidenceEffect}</dd></div></dl><p className="text-pretty text-sm leading-6">{incident.detail}</p><div className="border-t border-border pt-3"><p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Safe Next Step</p><p className="mt-1 text-sm">{incident.safeNextStep}</p></div></CardContent></Card>)}</div>}
    </section>
  );
}
